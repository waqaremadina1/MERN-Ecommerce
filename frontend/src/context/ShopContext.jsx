...
const updateQuantity = async (itemId, size, quantity) => {
    ...
    if (cartData[itemId] && cartData[itemId][size] !== undefined) {
        if (quantity > 0) {
            ...
            // Remove the size if the quantity is 0 or less
            delete cartData[itemId][size];
            // If no sizes are left, remove the entire item
        ...
        // If there's a valid token, update the backend
        if (token) {
            try {
        ...
        toast.error('Invalid item or size', 'error');
    }
};
